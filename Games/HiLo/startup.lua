...
debugLog("startup.lua: HiLo idle setup complete. Checking for updates...")
...
local function mainLoop()
  while true do
    ...
    if actionOrError == "play" then
      debugLog("startup.lua: Starting HiLo game...")
      local runOk, runErr = pcall(shell.run, "hilo.lua")
      if not runOk then
        debugLog("startup.lua: Error in hilo.lua: " .. tostring(runErr))
        alertLib.send("hilo.lua error: " .. tostring(runErr))
      ...
      debugLog("startup.lua: hilo.lua finished, returning to idle.")
    end
