---------------------------------------
-- startup.lua (Video Poker)
-- Idle animation until user clicks,
-- then launches videopoker.lua
...
debugLog("startup.lua: VideoPoker idle setup complete. Checking for updates...")
...
local function mainLoop()
  while true do
    ...
    if actionOrError == "play" then
      debugLog("startup.lua: Starting VideoPoker game...")
      local runOk, runErr = pcall(shell.run, "videopoker.lua")
      if not runOk then
        debugLog("startup.lua: Error in videopoker.lua: " .. tostring(runErr))
        alertLib.send("videopoker.lua error: " .. tostring(runErr))
      ...
      debugLog("startup.lua: videopoker.lua finished, returning to idle.")
    end
